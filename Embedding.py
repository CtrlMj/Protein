...
indices = np.random.randint(0, high=8900, size=300)
...
def create_model():
    model = Sequential()
    model.add(Embedding(21, 3, input_length=10553))
...
#From here starts the grad cam section
...
savedmodel = load_model("ConvModelrevised.h5")
...
classindx = np.argmax(preds[0])
outputclass = savedmodel.output[:, classindx]
print(outputclass)
...
last_conv_layer = savedmodel.get_layer("conv1d_5")
...
grads = K.gradients(outputclass, last_conv_layer.output)[0]
...
iterate = K.function([savedmodel.input], [pooled_grads, last_conv_layer.output[0]])
...
weighted_conv_layer_value = np.copy(conv_layer_output_value)
for i in range(64):
    weighted_conv_layer_value[:i] *= pooled_grads_value[i]

print(weighted_conv_layer_value.shape)


# In[35]:


heatmap = np.mean(weighted_conv_layer_value, axis=-1)
print(heatmap.shape)
...
resized_heatmap = np.resize(heatmap, len(instance))
...
plt.imshow(np.expand_dims(resized_heatmap, axis=0))
